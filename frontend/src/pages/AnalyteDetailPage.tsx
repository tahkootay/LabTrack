...
const AnalyteDetailPage: React.FC = () => {
  ...
  const { data: analyte, isLoading: analyteLoading } = useQuery({
    queryKey: ['analyte', analyteId],
    queryFn: () => analytesApi.getById(Number(analyteId)),
    enabled: !!analyteId,
  });

  const { data: history = [], isLoading: historyLoading, error } = useQuery({
    queryKey: ['analyte-history', analyteId],
    queryFn: () => resultsApi.getAnalyteHistory(Number(analyteId)),
    enabled: !!analyteId,
  });
  ...
  if (error) {
    return (
      <Container>
        <ErrorMessage 
          message="Не удалось загрузить историю показателя" 
        />
      </Container>
    );
  }
